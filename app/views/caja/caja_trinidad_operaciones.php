...
ob_start();
/**
 * ARCHIVO: caja_trinidad_operaciones.php
 * DESCRIPCIÓN: Gestión de operaciones de caja Trinidad (edición/eliminación)
 * 
 * FUNCIONALIDADES:
...
// Verificar permisos de escritura para Caja Trinidad (centro de costo 688)
$tiene_permiso_editar = false;
$tiene_permiso_leer = false;
$tiene_permiso_tramitar = false;

$sql_permiso = "SELECT permiso FROM permisos WHERE user_id = ? AND centro_costo_codigo = 688";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['delete_operation'])) {
    ...
    $sql_tramitado = "SELECT tramitado FROM caja_trinidad WHERE numero_operacion = ?";
    ...
    try {
        ...
        $sql_select = "SELECT entrada, salida FROM caja_trinidad WHERE numero_operacion = ?";
        ...
        $sql_delete = "DELETE FROM caja_trinidad WHERE numero_operacion = ?";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['edit_operation'])) {
    ...
    try {
        ...
        if ($diferencia != 0) {
            $sql_update_saldos = "UPDATE caja_trinidad 
                                 SET saldo = saldo + ? 
                                 WHERE numero_operacion > ?";
...
$centros_costo = [];
$sql_centros = "SELECT codigo, nombre FROM centros_costo WHERE E_Caja_Trinid = 1 OR S_Caja_Trinid = 1";
...
$por_pagina = 15;
$total_registros = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as total FROM caja_trinidad"))['total'] ?? 0;
...
$operaciones = [];
$sql = "SELECT ct.*, cc.nombre as centro_nombre, 
        IFNULL((SELECT 1 FROM relacion_cajas_principal_trinidad r WHERE r.operacion_trinidad = ct.numero_operacion LIMIT 1), 0) as relacionada
        FROM caja_trinidad ct
...
if (isset($_GET['editar'])) {
    ...
    $sql_relacionada = "SELECT 1 FROM relacion_cajas_principal_trinidad WHERE operacion_trinidad = ?";
    ...
    if ($result_relacionada->num_rows == 0) {
        ...
        if ($result_tramitado->num_rows > 0) {
            ...
            if ($operacion['tramitado'] == 0) {
                $sql_editar = "SELECT * FROM caja_trinidad WHERE numero_operacion = ?";
...
<div class="form-container">
    <h2>Caja Trinidad - Operaciones</h2>
                        ...
                        <td class="actions-cell">
                            <div class="table-action-buttons">
                                <?php if (!$row['relacionada'] && $row['tramitado'] == 0): ?>
                                    ...
                                    <form method="POST" action="caja_trinidad_operaciones.php" style="display: inline;">
    ...
    <div id="editFormContainer" class="sub-form" style="display: <?= $operacion_editar ? 'block' : 'none' ?>;">
        ...
        <form method="POST" action="caja_trinidad_operaciones.php">
            <input type="hidden" name="csrf_token" value="<?= $_SESSION['csrf_token'] ?>">
...
<script>
    ...
    function showEditForm(numeroOperacion) {
        window.location.href = `caja_trinidad_operaciones.php?editar=${numeroOperacion}`;
    }
    ...
    function hideEditForm() {
        window.location.href = "caja_trinidad_operaciones.php";
    }

...
ob_start();
/**
 * ARCHIVO: caja_panaderia_operaciones.php
 * DESCRIPCIÓN: Gestión de operaciones de caja panadería (edición/eliminación)
 * 
 * FUNCIONALIDADES:
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['delete_operation'])) {
    ...
    try {
        ...
        $sql_select = "SELECT entrada, salida FROM caja_panaderia WHERE numero_operacion = ?";
        ...
        $sql_delete = "DELETE FROM caja_panaderia WHERE numero_operacion = ?";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['edit_operation'])) {
    ...
    try {
        ...
        $sql_select = "SELECT 
                        ...
                        cp.salida, 
                        cp.saldo,
                        (SELECT saldo FROM caja_panaderia 
                         WHERE numero_operacion < ? 
        ...
        if ($diferencia != 0) {
            $sql_update_saldos = "UPDATE caja_panaderia 
                                 SET saldo = saldo + ? 
                                 WHERE numero_operacion > ?";
...
$centros_costo = [];
$sql_centros = "SELECT codigo, nombre FROM centros_costo WHERE E_Caja_Panad = 1 OR S_Caja_Panad = 1";
...
$sql = "SELECT cp.*, cc.nombre as centro_nombre, 
        IFNULL((SELECT 1 FROM relacion_cajas_principal_panaderia r WHERE r.operacion_panaderia = cp.numero_operacion LIMIT 1), 0) as relacionada
        FROM caja_panaderia cp
...
if (isset($_GET['editar'])) {
    ...
    $sql_relacionada = "SELECT 1 FROM relacion_cajas_principal_panaderia WHERE operacion_panaderia = ?";
...
<div class="form-container">
    <h2>Caja Panadería - Operaciones</h2>
    ...
    <div id="editFormContainer" class="sub-form" style="display: <?= $operacion_editar ? 'block' : 'none' ?>;">
        ...
        <form method="POST" action="caja_panaderia_operaciones.php">
            <input type="hidden" name="csrf_token" value="<?= $_SESSION['csrf_token'] ?>">
...
<script>
    ...
    function showEditForm(numeroOperacion) {
        window.location.href = `caja_panaderia_operaciones.php?editar=${numeroOperacion}`;
    }
    ...
    function hideEditForm() {
        window.location.href = "caja_panaderia_operaciones.php";
    }

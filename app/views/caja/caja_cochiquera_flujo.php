...
ob_start();
/**
 * ARCHIVO: caja_cochiquera_flujo.php
 * DESCRIPCIÓN: Visualización del flujo de caja de cochiquera
 * 
 * FUNCIONALIDADES:
 * - Visualización de listado completo del flujo de caja de cochiquera
...
// Verificar permisos para el centro de costo 738 (Caja cochiquera)
$tiene_permiso = false;
$puede_tramitar = $es_admin;

$sql_permisos = "SELECT permiso FROM permisos 
                WHERE user_id = ? AND centro_costo_codigo = 738 AND permiso IN ('leer', 'escribir', 'tramitar')";
...
if (!$tiene_permiso && !$es_admin) {
    echo "<div class='form-container'><p>No tiene permisos para visualizar la caja de cochiquera.</p></div>";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['actualizar_tramitado'])) {
    ...
    if (!isset($_SERVER['HTTP_X_REQUESTED_WITH']) || strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) != 'xmlhttprequest') {
        ...
        header("Location: caja_cochiquera_flujo.php");
        exit();
    }
}
...
// Consulta de operaciones de cochiquera con filtros
$entradas = [];
$salidas = [];
$sql_operaciones = "SELECT cp.*, cc.nombre as nombre_centro_costo
                   FROM caja_cochiquera cp
                   LEFT JOIN centros_costo cc ON cp.desde_para = cc.codigo
                   WHERE cp.fecha_operacion BETWEEN ? AND ? ";

// Aplicar filtro de tramitado
if ($filtro_tramitado === 'tramitado') {
    $sql_operaciones .= " AND cp.tramitado = 1";
} elseif ($filtro_tramitado === 'sin_tramitar') {
    $sql_operaciones .= " AND cp.tramitado = 0";
}

$sql_operaciones .= " ORDER BY cp.fecha_operacion DESC, cp.numero_operacion DESC";
...
<div class="form-container">
    <h2>Caja Cochiquera - Flujo</h2>

    <!-- Formulario de filtrado -->
    <form id="filtrosForm" method="GET" action="caja_cochiquera_flujo.php" class="filtros-form" style="display: <?= (isset($_GET['tramitado']) || isset($_GET['fecha_inicio']) || isset($_GET['fecha_fin']) || isset($_GET['tipo'])) ? 'none' : 'block' ?>;">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('.tramitado-checkbox').forEach(checkbox => {
            checkbox.addEventListener('change', function() {
                ...
                fetch('caja_cochiquera_flujo.php', {
                        method: 'POST',
                        body: formData
                    })

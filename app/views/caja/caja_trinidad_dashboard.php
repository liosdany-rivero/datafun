 ...
 * DESCRIPCIÓN: Dashboard principal para la gestión de la caja trinidad
 * FUNCIONALIDADES:
...
// Consultar permisos específicos para caja trinidad (código 800)
$stmt = $conn->prepare("SELECT permiso FROM permisos WHERE user_id = ? AND centro_costo_codigo = 688");
$stmt->bind_param("i", $_SESSION['user_id']);
$stmt->execute();
$result = $stmt->get_result();
$permisoCajaTrinidad = $result->fetch_assoc()['permiso'] ?? '';
$stmt->close();

// Botón Movimientos: solo para usuarios con permiso 'escribir' en centro de costo 800
if ($permisoCajaTrinidad === 'escribir') {
    $mostrarMovimientos = true;
}

// Botón Operaciones: para usuarios con permiso 'leer', 'escribir' o 'tramitar' en centro de costo 800
if (in_array($permisoCajaTrinidad, ['leer', 'escribir', 'tramitar'])) {
...
<div class="form-container">
    ...
    <div class="dashboard-buttons">
        <?php if ($mostrarMovimientos): ?>
            <a href="../caja/caja_trinidad_movimientos.php" class="dashboard-button">
        ...
        <?php if ($mostrarOperaciones): ?>
            <a href="../caja/caja_trinidad_operaciones.php" class="dashboard-button">
        ...
        <?php if ($mostrarFlujo): ?>
            <a href="../caja/caja_trinidad_flujo.php" class="dashboard-button">
                <span>Flujo</span>

...
ob_start();
/**
 * ARCHIVO: caja_cochiquera_movimientos.php
 * DESCRIPCIÓN: Gestión de operaciones de caja cochiquera del sistema
 * 
 * FUNCIONALIDADES:
 * - Creación de operaciones de entrada en caja cochiquera
 * - Creación de operaciones de salida en caja cochiquera
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['save_entrada'])) {
    ...
    if ($entrada <= 0) {
        ...
        header("Location: caja_cochiquera_movimientos.php");
        exit();
    }
    ...
    try {
        ...
        $saldo_anterior = 0;
        $sql_saldo = "SELECT saldo FROM caja_cochiquera ORDER BY numero_operacion DESC LIMIT 1";
        ...
        // Insertar en caja_cochiquera
        $sql = "INSERT INTO caja_cochiquera 
    ...
    try {
        ...
        if ($result_saldo && $result_saldo->num_rows > 0) {
            ...
            if ($saldo_anterior < $salida) {
                throw new Exception("Saldo insuficiente en caja cochiquera");
            }
        }
        ...
        error_log("Error en caja_cochiquera_movimientos.php: " . $e->getMessage());
    }
...
// Obtener centros de costo para entradas (E_Caja_Cochi = 1)
$centros_entrada = [];
$sql_centros_entrada = "SELECT codigo, nombre FROM centros_costo WHERE E_Caja_Cochi = 1";
$result_centros_entrada = mysqli_query($conn, $sql_centros_entrada);
while ($row = mysqli_fetch_assoc($result_centros_entrada)) {
    $centros_entrada[] = $row;
}

// Obtener centros de costo para salidas (S_Caja_Cochi = 1)
$centros_salida = [];
$sql_centros_salida = "SELECT codigo, nombre FROM centros_costo WHERE S_Caja_Cochi = 1";
...
<div class="form-container">
    ...
    <div id="entradaFormContainer" class="sub-form" style="display: none;">
        ...
        <form method="POST" action="caja_cochiquera_movimientos.php">
...
<script>
    ...
    function showContadorDinero(targetField) {
        ...
        window.location.href = `contador_dinero.php?target=${targetField}&source=caja_cochiquera_movimientos.php&formType=${formType}`;
    }

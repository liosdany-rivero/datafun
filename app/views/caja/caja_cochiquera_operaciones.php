...
ob_start();
/**
 * ARCHIVO: caja_cochiquera_operaciones.php
 * DESCRIPCIÓN: Gestión de operaciones de caja cochiquera (edición/eliminación)
 * 
 * FUNCIONALIDADES:
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['delete_operation'])) {
    ...
    $sql_tramitado = "SELECT tramitado FROM caja_cochiquera WHERE numero_operacion = ?";
    ...
    try {
        ...
        $sql_select = "SELECT entrada, salida FROM caja_cochiquera WHERE numero_operacion = ?";
        ...
        $sql_delete = "DELETE FROM caja_cochiquera WHERE numero_operacion = ?";
        ...
        if ($operacion['entrada'] > 0) {
            ...
            $sql_update = "UPDATE caja_cochiquera 
                          SET saldo = saldo - ? 
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['edit_operation'])) {
    ...
    try {
        ...
        $sql_select = "SELECT 
                        ...
                        cp.salida, 
                        cp.saldo,
                        (SELECT saldo FROM caja_cochiquera 
                         WHERE numero_operacion < ? 
                         ORDER BY numero_operacion DESC LIMIT 1) as saldo_anterior
                      FROM caja_cochiquera cp
        ...
        if ($diferencia != 0) {
            $sql_update_saldos = "UPDATE caja_cochiquera 
                                 SET saldo = saldo + ? 
                                 WHERE numero_operacion > ?";
...
$centros_costo = [];
$sql_centros = "SELECT codigo, nombre FROM centros_costo WHERE E_Caja_Cochi = 1 OR S_Caja_Cochi = 1";
...
$por_pagina = 15;
$total_registros = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as total FROM caja_cochiquera"))['total'] ?? 0;
...
$sql = "SELECT cp.*, cc.nombre as centro_nombre, 
        IFNULL((SELECT 1 FROM relacion_cajas_principal_cochiquera r WHERE r.operacion_cochiquera = cp.numero_operacion LIMIT 1), 0) as relacionada
        FROM caja_cochiquera cp
...
if (isset($_GET['editar'])) {
    ...
    $sql_relacionada = "SELECT 1 FROM relacion_cajas_principal_cochiquera WHERE operacion_cochiquera = ?";
    ...
    if ($result_relacionada->num_rows == 0) {
        ...
        if ($result_tramitado->num_rows > 0) {
            ...
            if ($operacion['tramitado'] == 0) {
                $sql_editar = "SELECT * FROM caja_cochiquera WHERE numero_operacion = ?";
...
<div class="form-container">
    <h2>Caja Cochiquera - Operaciones</h2>
                        ...
                        <td class="actions-cell">
                            <div class="table-action-buttons">
                                <?php if (!$row['relacionada'] && $row['tramitado'] == 0): ?>
                                    ...
                                    <form method="POST" action="caja_cochiquera_operaciones.php" style="display: inline;">
    ...
    <div id="editFormContainer" class="sub-form" style="display: <?= $operacion_editar ? 'block' : 'none' ?>;">
        ...
        <form method="POST" action="caja_cochiquera_operaciones.php">
            <input type="hidden" name="csrf_token" value="<?= $_SESSION['csrf_token'] ?>">
...
<script>
    ...
    function showEditForm(numeroOperacion) {
        window.location.href = `caja_cochiquera_operaciones.php?editar=${numeroOperacion}`;
    }
    ...
    function hideEditForm() {
        window.location.href = "caja_cochiquera_operaciones.php";
    }

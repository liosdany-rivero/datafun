 ...
 * DESCRIPCIÓN: Dashboard principal para la gestión de la caja principal
 * FUNCIONALIDADES:
...
// Consultar permisos específicos para Caja Principal (código 800)
$stmt = $conn->prepare("SELECT permiso FROM permisos WHERE user_id = ? AND centro_costo_codigo = 800");
$stmt->bind_param("i", $_SESSION['user_id']);
$stmt->execute();
$result = $stmt->get_result();
$permisoCajaPrincipal = $result->fetch_assoc()['permiso'] ?? '';
$stmt->close();

// Botón Movimientos: solo para usuarios con permiso 'escribir' en centro de costo 800
if ($permisoCajaPrincipal === 'escribir') {
    $mostrarMovimientos = true;
}

// Botón Operaciones: para usuarios con permiso 'leer', 'escribir' o 'tramitar' en centro de costo 800
if (in_array($permisoCajaPrincipal, ['leer', 'escribir', 'tramitar'])) {
...
<div class="form-container">
    <h2>Caja Principal - Dashboard</h2>
    ...
    <div class="dashboard-buttons">
        <?php if ($mostrarMovimientos): ?>
            <a href="../caja/caja_principal_movimientos.php" class="dashboard-button">
        ...
        <?php if ($mostrarOperaciones): ?>
            <a href="../caja/caja_principal_operaciones.php" class="dashboard-button">
        ...
        <?php if ($mostrarFlujo): ?>
            <a href="../caja/caja_principal_flujo.php" class="dashboard-button">
                <span>Flujo</span>

...
ob_start();
/**
 * ARCHIVO: caja_trinidad_movimientos.php
 * DESCRIPCIÓN: Gestión de operaciones de caja Trinidad del sistema
 * 
 * FUNCIONALIDADES:
 * - Creación de operaciones de entrada en caja Trinidad
 * - Creación de operaciones de salida en caja Trinidad
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['save_entrada'])) {
    ...
    if ($entrada <= 0) {
        ...
        header("Location: caja_trinidad_movimientos.php");
        exit();
    }
    ...
    try {
        ...
        $saldo_anterior = 0;
        $sql_saldo = "SELECT saldo FROM caja_trinidad ORDER BY numero_operacion DESC LIMIT 1";
        ...
        // Insertar en caja_trinidad
        $sql = "INSERT INTO caja_trinidad 
    ...
    try {
        ...
        if ($result_saldo && $result_saldo->num_rows > 0) {
            ...
            if ($saldo_anterior < $salida) {
                throw new Exception("Saldo insuficiente en caja Trinidad");
            }
        }
        ...
        error_log("Error en caja_trinidad_movimientos.php: " . $e->getMessage());
    }
...
// Obtener centros de costo para entradas (E_Caja_Trinid = 1)
$centros_entrada = [];
$sql_centros_entrada = "SELECT codigo, nombre FROM centros_costo WHERE E_Caja_Trinid = 1";
$result_centros_entrada = mysqli_query($conn, $sql_centros_entrada);
while ($row = mysqli_fetch_assoc($result_centros_entrada)) {
    $centros_entrada[] = $row;
}

// Obtener centros de costo para salidas (S_Caja_Trinid = 1)
$centros_salida = [];
$sql_centros_salida = "SELECT codigo, nombre FROM centros_costo WHERE S_Caja_Trinid = 1";
...
<div class="form-container">
    ...
    <div id="entradaFormContainer" class="sub-form" style="display: none;">
        ...
        <form method="POST" action="caja_trinidad_movimientos.php">
...
<script>
    ...
    function showContadorDinero(targetField) {
        ...
        window.location.href = `contador_dinero.php?target=${targetField}&source=caja_trinidad_movimientos.php&formType=${formType}`;
    }

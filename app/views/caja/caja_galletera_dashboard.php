 ...
 * DESCRIPCIÓN: Dashboard principal para la gestión de la caja galletera
 * FUNCIONALIDADES:
...
// Consultar permisos específicos para caja galletera (código 800)
$stmt = $conn->prepare("SELECT permiso FROM permisos WHERE user_id = ? AND centro_costo_codigo = 708");
$stmt->bind_param("i", $_SESSION['user_id']);
$stmt->execute();
$result = $stmt->get_result();
$permisoCajaGalletera = $result->fetch_assoc()['permiso'] ?? '';
$stmt->close();

// Botón Movimientos: solo para usuarios con permiso 'escribir' en centro de costo 800
if ($permisoCajaGalletera === 'escribir') {
    $mostrarMovimientos = true;
}

// Botón Operaciones: para usuarios con permiso 'leer', 'escribir' o 'tramitar' en centro de costo 800
if (in_array($permisoCajaGalletera, ['leer', 'escribir', 'tramitar'])) {
...
<div class="form-container">
    <h2>Caja Galletera - Dashboard</h2>
    ...
    <div class="dashboard-buttons">
        <?php if ($mostrarMovimientos): ?>
            <a href="../caja/caja_galletera_movimientos.php" class="dashboard-button">
        ...
        <?php if ($mostrarOperaciones): ?>
            <a href="../caja/caja_galletera_operaciones.php" class="dashboard-button">
        ...
        <?php if ($mostrarFlujo): ?>
            <a href="../caja/caja_galletera_flujo.php" class="dashboard-button">
                <span>Flujo</span>

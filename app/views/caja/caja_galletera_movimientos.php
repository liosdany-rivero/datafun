...
ob_start();
/**
 * ARCHIVO: caja_galletera_movimientos.php
 * DESCRIPCIÓN: Gestión de operaciones de caja galletera del sistema
 * 
 * FUNCIONALIDADES:
 * - Creación de operaciones de entrada en caja galletera
 * - Creación de operaciones de salida en caja galletera
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['save_entrada'])) {
    ...
    if ($entrada <= 0) {
        ...
        header("Location: caja_galletera_movimientos.php");
        exit();
    }
    ...
    try {
        ...
        $saldo_anterior = 0;
        $sql_saldo = "SELECT saldo FROM caja_galletera ORDER BY numero_operacion DESC LIMIT 1";
        ...
        // Insertar en caja_galletera
        $sql = "INSERT INTO caja_galletera 
    ...
    try {
        ...
        if ($result_saldo && $result_saldo->num_rows > 0) {
            ...
            if ($saldo_anterior < $salida) {
                throw new Exception("Saldo insuficiente en caja galletera");
            }
        }
        ...
        error_log("Error en caja_galletera_movimientos.php: " . $e->getMessage());
    }
...
// Obtener centros de costo para entradas (E_Caja_Gallet = 1)
$centros_entrada = [];
$sql_centros_entrada = "SELECT codigo, nombre FROM centros_costo WHERE E_Caja_Gallet = 1";
$result_centros_entrada = mysqli_query($conn, $sql_centros_entrada);
while ($row = mysqli_fetch_assoc($result_centros_entrada)) {
    $centros_entrada[] = $row;
}

// Obtener centros de costo para salidas (S_Caja_Gallet = 1)
$centros_salida = [];
$sql_centros_salida = "SELECT codigo, nombre FROM centros_costo WHERE S_Caja_Gallet = 1";
...
<div class="form-container">
    ...
    <div id="entradaFormContainer" class="sub-form" style="display: none;">
        ...
        <form method="POST" action="caja_galletera_movimientos.php">
...
<script>
    ...
    function showContadorDinero(targetField) {
        ...
        window.location.href = `contador_dinero.php?target=${targetField}&source=caja_galletera_movimientos.php&formType=${formType}`;
    }

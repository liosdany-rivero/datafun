...
ob_start();
/**
 * ARCHIVO: caja_panaderia_flujo.php
 * DESCRIPCIÓN: Visualización del flujo de caja de panadería
 * 
 * FUNCIONALIDADES:
 * - Visualización de listado completo del flujo de caja de panadería
...
// Verificar permisos para el centro de costo 728 (Caja panadería)
$tiene_permiso = false;
$puede_tramitar = $es_admin;

$sql_permisos = "SELECT permiso FROM permisos 
                WHERE user_id = ? AND centro_costo_codigo = 728 AND permiso IN ('leer', 'escribir', 'tramitar')";
...
if (!$tiene_permiso && !$es_admin) {
    echo "<div class='form-container'><p>No tiene permisos para visualizar la caja de panadería.</p></div>";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['actualizar_tramitado'])) {
    ...
    if ($es_admin || $puede_tramitar) {
        ...
        $sql_update = "UPDATE caja_panaderia 
                      SET tramitado = ? 
    ...
    if (!isset($_SERVER['HTTP_X_REQUESTED_WITH']) || strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) != 'xmlhttprequest') {
        ...
        header("Location: caja_panaderia_flujo.php");
        exit();
    }
}
...
// Consulta de operaciones de panadería con filtros
$entradas = [];
$salidas = [];
...
<div class="form-container">
    <h2>Caja Panadería - Flujo</h2>

    <!-- Formulario de filtrado -->
    <form id="filtrosForm" method="GET" action="caja_panaderia_flujo.php" class="filtros-form" style="display: <?= (isset($_GET['tramitado']) || isset($_GET['fecha_inicio']) || isset($_GET['fecha_fin']) || isset($_GET['tipo'])) ? 'none' : 'block' ?>;">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('.tramitado-checkbox').forEach(checkbox => {
            checkbox.addEventListener('change', function() {
                ...
                fetch('caja_panaderia_flujo.php', {
                        method: 'POST',
                        body: formData
                    })

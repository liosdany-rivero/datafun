 ...
 * DESCRIPCIÓN: Dashboard principal para la gestión de la caja panaderia
 * FUNCIONALIDADES:
...
// Consultar permisos específicos para caja panaderia (código 800)
$stmt = $conn->prepare("SELECT permiso FROM permisos WHERE user_id = ? AND centro_costo_codigo = 728");
$stmt->bind_param("i", $_SESSION['user_id']);
$stmt->execute();
$result = $stmt->get_result();
$permisoCajaPanaderia = $result->fetch_assoc()['permiso'] ?? '';
$stmt->close();

// Botón Movimientos: solo para usuarios con permiso 'escribir' en centro de costo 800
if ($permisoCajaPanaderia === 'escribir') {
    $mostrarMovimientos = true;
}

// Botón Operaciones: para usuarios con permiso 'leer', 'escribir' o 'tramitar' en centro de costo 800
if (in_array($permisoCajaPanaderia, ['leer', 'escribir', 'tramitar'])) {
...
<div class="form-container">
    <h2>Caja Panadería - Dashboard</h2>
    ...
    <div class="dashboard-buttons">
        <?php if ($mostrarMovimientos): ?>
            <a href="../caja/caja_panaderia_movimientos.php" class="dashboard-button">
        ...
        <?php if ($mostrarOperaciones): ?>
            <a href="../caja/caja_panaderia_operaciones.php" class="dashboard-button">
        ...
        <?php if ($mostrarFlujo): ?>
            <a href="../caja/caja_panaderia_flujo.php" class="dashboard-button">
                <span>Flujo</span>
